...
class BouncyGUI(Frame):
    """Application window for the bounciness calculator."""
    def _compute(self):
        """Gathers the information used for the calculations of the bouncy,
        distance traveled in total."""
        initialHeight = self._heightVar.get()
        bounceyIndex = 0.6
        numberOfBounce = self._bounceVar.get()
        bounceHeight = initialHeight * bounceyIndex
        totalDistance = initialHeight
        for i in range(1,(int(numberOfBounce))):
    ...
    def __init__(self):
        ...
        Frame.__init__(self)
        self.master.title("Bounciness Calculator")
        self.grid()
